...
| Sync L4 RPC wrappers (still park on MeshControl) | MeshControlPool (default 1) | Media-relay Async APIs + empty io_pump under MeshPump; SoftMigrate / circuit hop / punch still sync-park on control |
...
| **Sync L4 RPC still parks until reply** | Product chat/history/blob/directory/reachability/announce are async; media-relay has Async APIs. SoftMigrate quote/attach + AmpCircuitHopReach + CallStack punch still sync on MeshControl. Test sync wrappers remain. |
