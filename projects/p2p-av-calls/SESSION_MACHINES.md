...
| Sync L4 RPC wrappers (still park on MeshControl) | MeshControlPool (default 1) | Product chat/history/blob fetch+push/directory/TailSync/UI sync async; announce PublishAndPush* sync wrappers unused by UI |
...
| **Sync L4 RPC still parks until reply** | Product paths (chat send, history sync, attachment fetch/push, directory) are async. Unused announce PublishAndPush sync helpers and test sync wrappers remain. Not in call-media SM scope. |
