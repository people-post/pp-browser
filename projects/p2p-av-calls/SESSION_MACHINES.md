...
| Sync L4 RPC wrappers (still park on MeshControl) | MeshControlPool (default 1) | Channel-open is `AmpScheduleWhenChannelOpen` + MeshPump; product chat send uses `SendEnvelopeAsync`; directory refresh uses `ListMeshNodesAsync` |
...
| **Sync L4 RPC still parks until reply** | Channel-open is `AmpScheduleWhenChannelOpen` (dial-back/punch/chat/blob/history/broadcast/announce/DHT/directory). Product direct-chat send + directory refresh are async; blob/history/broadcast/announce sync wrappers still SettledWait. Not in call-media SM scope. |
