...
| Sync L4 RPC wrappers (still park on MeshControl) | MeshControlPool (default 1) | Product chat/history/blob/directory/TailSync/UI sync/reachability/announce push async; circuit hop + punch + media-relay SettledWait remain |
...
| **Sync L4 RPC still parks until reply** | Product chat/history/blob/directory/reachability/announce are async. Circuit hop reach, CallStack punch, and media-relay quote/attach still SettledWait; test sync wrappers remain. Not in call-media SM scope. |
