...
**Context:** Separating call session from conversations is desired, but `CallSessionManager` ↔ `MeshMessagingService` would cycle if `pp_feature_calls` and `pp_feature_conversations` both exist today.

**Decision:**

1. **f4v1:** nest under `feature/conversations/calls/` (same `pp_feature_conversations` target) — discoverability only.
