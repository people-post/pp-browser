...
| Direct | normal text | `MeshDeliveryOrchestrator::SendUserMessage` → relay (or direct transport) |
...
Persist `transport` (`local` / `relay` / `direct`) at send/receive in `MeshDeliveryOrchestrator` (and future libp2p layer). E2E vs public **thread shell** styling (`.chat-shell--e2e`) in v2b.
...
Implementation lives in `src/domain/mesh/` + `MeshDeliveryOrchestrator` — not in `IThreadStore`.
