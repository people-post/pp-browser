...
| Area | State |
|------|-------|
| IPv6 | Dual-stack Amp listen (`::` when global v6 present); `AdvertisedListenMultiaddrs` for DHT/directory/invite/SoftMigrate/status/settings; register PreferredMultiaddr as global `/ip6` when mixed; Reachable-via-v6 = `has_global_ipv6` + `dial_back_ok`; `MultiaddrHasPublicDialHost` gates PreferLocal/public hops; call_probe `/ip6` ParseListen + `::`→`::1` rewrite |
