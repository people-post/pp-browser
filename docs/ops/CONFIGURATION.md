...
- **`libp2p`** — mesh role and Amp underlay policy. `node_enabled` (desktop; ignored on mobile) selects Node vs Client hosting posture ([p2p-mesh N001](../../projects/p2p-mesh/DECISIONS.md)). **`enable_amp_stack`** (default **true**) is required for the peer mesh: Amp UDP + MSH + channels ([adp D10](../../projects/adp/PHASES.md)). Amp bind/start failure **fails mesh start** (no TCP underlay fallback). Set `enable_amp_stack=false` to leave peer mesh off. Optional **`amp_udp_port`** (0 = ephemeral). LAN mDNS TXT includes `amp_udp=` so peers can build ADP multiaddrs. Empty `bootstrap_peers` fills the Brief seed for SoftMigrate hop ranking. Org **`pp-node`** hosts Amp circuit/media-relay when capabilities are on. Me → Network shows Help-the-network, Amp listen, and Inbound via Amp dial-back (D8; needs ADP `bootstrap_peers`). Contacts may store dialable ADP `multiaddrs` (`/ip4/…/udp/…/adp/1.0.0/p2p/<PeerId>`).
