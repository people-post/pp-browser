...
- `boost/CMakeLists.txt` — pp-browser wrapper using Boost CMake superproject; unified `boost/` include for compiled libs; vendored build no longer pulls `filesystem`, `program_options`, or `algorithm` (replaced with STL / local helpers in the fork and qtils)
