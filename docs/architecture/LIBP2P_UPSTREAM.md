...
- `boost/CMakeLists.txt` — pp-browser wrapper using Boost CMake superproject; unified `boost/` include for compiled libs; vendored build no longer pulls `filesystem`, `program_options`, `algorithm`, or `random` (replaced with STL / local helpers; CSPRNG is `StdRandomGenerator`); also dropped Boost.Variant / Boost.Container small/static_vector from fork sources
