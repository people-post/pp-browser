...
**Punch / ACP:** Mux frame handlers only `PostToIo` (`PostStrand`). Introducer connect is fully async. Sync-window burst uses `BurstDialCandidatesAsync` whenever `IoPost` is set (tests + product). `AbortInflightDial` + result complete settle via `SchedulePark` / `DrainParkWork` onto the waiter IoPump stack when `IoPump` is present — never under `MeshRuntime::DrainPostedIo` (Windows SEH). Product MeshPump passes empty `IoPump` and settles on the IO strand. Sync `BurstDialCandidates` is test-only without `IoPost`.
