...
| Item | Location | Notes |
|------|----------|-------|
| Remaining sync L4 parks | outbound peer blob push | UI sync (`RunSyncOnIo`), TailSync, attachment peer fetch, history/blob/announce/broadcast Async APIs landed; `PushChatBlob` sync wrapper remains for upload |
