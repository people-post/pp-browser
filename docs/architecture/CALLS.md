...
**Vocabulary boundary ([V048](../../projects/p2p-av-calls/DECISIONS.md#v048--hop-planner-vocabulary-no-lifecycle-concepts-downward)):** Stack is the composition root; Topology must not speak Lifecycle (`CallMediaStatus`, chrome Status writers). It needs only arming, cancel epoch, hop-native progress, and seat bind — Stack projects hop progress upward to Lifecycle. Today’s `CallTopologyLifecyclePorts` still smuggle Lifecycle enums; follow-up replaces them with hop-native arming ports.
