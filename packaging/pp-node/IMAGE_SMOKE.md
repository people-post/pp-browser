...
|-------|----------------|---------|--------|
| **L0** | Process up; `/healthz` + `/status`; caps **started** (`circuit_relay` / `media_relay` flags) | `scripts/test/pp_node_image_smoke.sh` | **Done** |
| **L1** | From outside the hop: dial peer; **media** `RequestQuote`; **circuit** bridge + payload to a local target | `pp-node-probe` + `scripts/test/pp_node_relay_smoke.sh` | **Done** (scaffold) |
| **L2** | **N-FANOUT:** hop in container + two client hosts in probe; attach×2 + frame fan-out | `docker-compose.relay-smoke.yml` + `pp-node-probe --mode media-fanout` + `scripts/test/pp_node_fanout_smoke.sh` | **Done** (scaffold) |
| **N-DHT** | Two local `pp-node`s with `PP_NODE_CAP_DHT=1` discover each other’s ADP addrs (no Brief HTTP) | `scripts/test/pp_node_dht_smoke.sh` | **Done** (lab) |
| **N-CAP** | Soft media attach capacity curve (N=4 cheap; sweep via `--suite cap`) | `pp-node-probe --mode media-cap` + `scripts/test/pp_node_cap_smoke.sh` | **Done** (soft SLO N≤8; 12/16 informational) |
...
| **Hard lab** | Forced A↛B nets + impairments / discovery (Tier C) | [HARD_LAB.md](HARD_LAB.md) + `--suite hard` / `hard-w2` | **Wave 1–2 scaffold**; Wave 3+ open |
...
[`scripts/test/pp_local_test.sh`](../../scripts/test/pp_local_test.sh) starts/stops the **relay-smoke** hop and runs L0–L2 / N-CAP (and optional unit/call/cap/soak/chaos/call-hop/mix suites). Individual `pp_*_smoke.sh` scripts remain for CI when the hop is already up.

```bash
# After packaging dist/pp-node/docker (see BUILD.md):
./scripts/test/pp_local_test.sh run --suite node     # up hop + L0/L1/fanout/cap N=4; hop stays up
./scripts/test/pp_local_test.sh run --suite cap      # media sweep 4,8,12,16 + circuit-cap
./scripts/test/pp_local_test.sh run --suite soak     # 120s churn (PP_NODE_SOAK_SEC=3600 weekly)
./scripts/test/pp_local_test.sh run --suite chaos    # kill-client / restart / pause
./scripts/test/pp_local_test.sh run --suite call-hop # B-CALL-HOP thin client
./scripts/test/pp_local_test.sh run --suite msg-call-hop # chat during hop call (same pair)
./scripts/test/pp_local_test.sh run --suite mix      # B-MIX + N-MIX + same-session hop chat (nightly)
./scripts/test/pp_local_test.sh status
./scripts/test/pp_local_test.sh stop                 # keep volume
./scripts/test/pp_local_test.sh clear                # down -v
```
...
```bash
./scripts/test/pp_node_relay_smoke.sh          # L0 then L1
...
docker compose -f packaging/pp-node/docker-compose.relay-smoke.yml up -d --build
cmake --build build --target pp-node-probe -j
./scripts/test/pp_node_fanout_smoke.sh   # L0 then media-fanout
```
