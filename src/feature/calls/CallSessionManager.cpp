...
Roe<void> CallSessionManager::AcceptInvite(const std::string& call_id,
  ...
  if (lifecycle_ports_.accepting_call_id && lifecycle_ports_.active_call_id) {
    const std::string accepting = lifecycle_ports_.accepting_call_id();
    const std::string active = lifecycle_ports_.active_call_id();
...
Roe<void> CallSessionManager::EndCallLocal(CallSession& session, const std::optional<int64_t>& duration_ms) {
  ...
  if (lifecycle_ports_.active_call_id && lifecycle_ports_.apply_remote_ended &&
      lifecycle_ports_.active_call_id() == session.call_id) {
    lifecycle_ports_.apply_remote_ended(session.call_id);
  }
  NotifyRingChanged();
  return {};
}
...
void CallSessionManager::SweepExpiredInvites() {
  ...
  if (auto pending = sessions_.ListPendingInvitesForInvitee(*local); pending) {
    for (PendingCallInvite& invite : *pending) {
      ...
      } else if (lifecycle_ports_.active_call_id && lifecycle_ports_.apply_remote_ended &&
                 lifecycle_ports_.active_call_id() == invite.call_id) {
        lifecycle_ports_.apply_remote_ended(invite.call_id);
      }
      changed = true;
    }
  }
  ...
  if (lifecycle_ports_.is_outbound_calling && lifecycle_ports_.is_outbound_calling() &&
      !media_.IsActive()) {
    auto active = ActiveLocalCall();
    if (active && active->has_value() &&
      ...
      if (lifecycle_ports_.active_call_id && lifecycle_ports_.active_call_id() == call_id) {
...
Roe<void> CallSessionManager::HandleInboundAccept(const std::string& detail_json,
  ...
  if (session && session->has_value()) {
    ...
    if (!topology_.OnRemoteAcceptJoined(accept->call_id, n_joined, identity)) {
      if (lifecycle_ports_.set_direct_connecting) {
        lifecycle_ports_.set_direct_connecting(accept->call_id);
      }
