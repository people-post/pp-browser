...
void RunWorker(const AmpPeerAnnounceTransport::WorkerPost& post_worker, std::function<void()> task) {
  if (post_worker) {
...
struct AmpPeerAnnounceTransport::Impl {
...
AmpPeerAnnounceTransport::~AmpPeerAnnounceTransport() {
  Stop();
}

void AmpPeerAnnounceTransport::Start() {
  if (started_) {
    return;
  }
  started_ = true;
...
void AmpPeerAnnounceTransport::SetOnTipIngested(OnTipIngested cb) {
  std::lock_guard lock(impl_->resolver_mutex);
