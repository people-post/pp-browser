...
PushDeviceRegistration MakeRegistration(const std::string& token) {
...
Roe<void> PushDeviceCoordinator::UnregisterCurrent() {
  if (!MessagingHub::Instance().IsMessagingReady()) {
    return {};
  }
  IPushDeviceClient* client = MessagingHub::Instance().PushDevices();
  if (client == nullptr || !IPushDeviceRegistrar::Instance().IsSupported()) {
    return {};
  }
  auto token = IPushDeviceRegistrar::Instance().GetPushToken();
  PushDeviceRegistration reg = MakeRegistration(token ? *token : std::string{});
...
Roe<void> PushDeviceCoordinator::SyncWithPreference(const bool show_notifications) {
  ...
  PushDeviceRegistration reg = MakeRegistration(*token);
