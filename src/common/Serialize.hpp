...
struct WireLenUtf8 {
  ...
  template <typename Archive> void serialize(Archive &ar) { ar & value; }
};

/** u64 BE length + opaque bytes — pp Binary Wire Profile. */
struct WireLenBytes {
...
inline void floatingPointToBigEndian(FloatType value, uint8_t *bytes) {
  static_assert(
      (std::is_same_v<FloatType, float> && std::is_same_v<IntType, uint32_t>) ||
