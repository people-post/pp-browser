...
namespace libp2p::security::secio {
  ...
  class ExchangeMessageMarshallerImpl : public ExchangeMessageMarshaller {
   public:
    ...
    wire::SecioExchangeWire handyToWire(
        const ExchangeMessage &msg) const override;

    ExchangeMessage wireToHandy(
        const wire::SecioExchangeWire &wire_msg) const override;

    outcome::result<std::vector<uint8_t>> marshal(
        const ExchangeMessage &msg) const override;

    outcome::result<ExchangeMessage> unmarshal(BytesIn msg_bytes) const override;
  };
