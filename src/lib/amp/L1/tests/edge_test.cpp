...
std::vector<uint8_t> SealPacket(const pbr::adp::PeerKey& key, const pbr::adp::WirePacket& pkt) {
...
TEST_F(AdpEdgeTest, ReplayRejectOnWireBestEffort) {
  ...
  size_t got = 0;
  (*cb)->OnMessage([&](const pbr::adp::Message&) { ++got; });
...
TEST_F(AdpEdgeTest, LooksAliveTimeout) {
  ...
  p.clock->Advance(pbr::adp::kAliveTimeoutMs + 1);
  EXPECT_FALSE((*cb)->LooksAlive(p.clock->NowMs()));
}
...
TEST_F(AdpEdgeTest, UpgradeBinderRejectsOldKey) {
  ...
  pbr::adp::WirePacket old_pkt;
  old_pkt.type = pbr::adp::PacketType::DataBestEffort;
  old_pkt.assoc = Aid();
  old_pkt.seq = 1;
  ...
  pbr::adp::WirePacket new_pkt = old_pkt;
  new_pkt.seq = 2;
  new_pkt.payload = {'b'};
...
TEST_F(AdpEdgeTest, MintAssocIdUnique) {
  ...
  ASSERT_TRUE(c1);
  EXPECT_NE((*c1)->Id().bytes, pbr::adp::AssocId{}.bytes);
...
TEST_F(AdpEdgeTest, PayloadTooLargeOnSend) {
  ...
  auto ca = p.ep_a->Open(op);
  ASSERT_TRUE(ca);
  std::vector<uint8_t> big(static_cast<size_t>(pbr::adp::kMaxPayload) + 1, 0xab);
  const auto err = (*ca)->Send(pbr::adp::QosClass::BestEffort, big);
  EXPECT_FALSE(err);
...
TEST_F(AdpEdgeTest, ReliableWindowFull) {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    ASSERT_TRUE((*ca)->Send(pbr::adp::QosClass::Reliable, std::span<const uint8_t>(&b, 1)));
  }
  const uint8_t extra = 'x';
  const auto err = (*ca)->Send(pbr::adp::QosClass::Reliable, std::span<const uint8_t>(&extra, 1));
  EXPECT_FALSE(err);
...
TEST_F(AdpEdgeTest, AcceptHandlerFiresOnce) {
  ...
  p.ep_b->SetAcceptEnabled(true);
  int accepts = 0;
  p.ep_b->SetAcceptHandler([&](std::shared_ptr<pbr::adp::Connection>) { ++accepts; });
