...
std::string ConfigDirImpl() {
  ...
  if (const std::string xdg = ReadEnv("XDG_CONFIG_HOME"); !xdg.empty()) {
    return (std::filesystem::path(xdg) / product).string();
  }
  const std::string home = HomeDir();
  if (!home.empty()) {
    return (std::filesystem::path(home) / ".config" / product).string();
  }
...
std::string DataDirImpl(const std::string& override_path) {
  ...
  const std::string home = HomeDir();
  if (!home.empty()) {
    return (std::filesystem::path(home) / ".local" / "share" / product).string();
  }
